    ...
    <main className="min-h-screen bg-[#f4f4f1] text-zinc-950">
      ...
      <section className="mx-auto max-w-[1500px] px-5 py-8 sm:px-10">
        <div className="mb-6 grid grid-cols-2 gap-3 sm:max-w-md">
          ...
          <div className="border border-zinc-300 bg-white p-4"><span className="text-xs text-zinc-500">NEW</span><strong className="mt-1 block text-2xl text-red-700">{orders.filter(order => order.status === "new").length}</strong></div>
        </div>
        ...
        {orders.length > 0 && <div className="border border-zinc-300 bg-white">
          <Table>
            ...
            <TableBody>{orders.map(order => <TableRow key={order.id}>
              ...
              <TableCell className="align-top"><Select value={order.status} onValueChange={(value) => updateStatus(order.id, value)}><SelectTrigger aria-label={`Status for ${order.id}`}><SelectValue /></SelectTrigger><SelectContent>{statuses.map(status => <SelectItem key={status} value={status}>{status.toUpperCase()}</SelectItem>)}</SelectContent></Select></TableCell>
            </TableRow>)}</TableBody>
          </Table>
        </div>}
      </section>
